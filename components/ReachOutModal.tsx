...
import { mockReachOutTemplates } from '../constants';
...
const ReachOutModal: React.FC<ReachOutModalProps> = ({ donor, onClose, onSend }) => {
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200 flex flex-col max-h-[90vh] animate-slide-up">
                <div className="flex justify-between items-center p-6 border-b border-gray-200 flex-shrink-0">
                    <h3 className="text-2xl font-bold text-gray-900">Send Reach Out to {donor.name}</h3>
                ...
                <div className="p-6 overflow-y-auto space-y-6 bg-gray-50">
                    <div>
                        <label htmlFor="message" className="block text-sm font-medium text-gray-700 mb-1.5">Personalized Message</label>
                        <textarea
                            id="message"
                            rows={8}
                            value={message}
                            ...
                            placeholder="Write a personal note to encourage the donor..."
                        />
                    </div>
                    <div>
                        <h4 className="text-sm font-medium text-gray-700 mb-2">Use a template</h4>
                        <div className="flex flex-wrap gap-2">
                            {mockReachOutTemplates.map(template => (
                                <button
                                    key={template.id}
                                    onClick={() => handleTemplateClick(template)}
                                    className="bg-amber-100 text-amber-800 hover:bg-amber-200 text-xs font-semibold py-1.5 px-3 rounded-full transition-colors"
                                >
                                    {template.title}
                                </button>
                            ))}
